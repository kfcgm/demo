<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="建材类型" prop="jiancaileixing">
          <el-input v-model="ruleForm.jiancaileixing" 
              placeholder="建材类型" clearable  :readonly="ro.jiancaileixing"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  data() {
	...
	jiancaileixing : false,
            ...
            ruleForm: {
                          ...
                          rules: {
                  jiancaileixing: [
                            { required: true, message: '建材类型不能为空', trigger: 'blur' },
                                    	                                                              ],
              }
    };
  },
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `jiancaileixing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jiancaileixingCrossAddOrUpdateFlag = false;
                  this.parent.search();
