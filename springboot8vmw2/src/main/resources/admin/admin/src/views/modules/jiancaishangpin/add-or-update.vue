<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="建材名称" prop="jiancaimingcheng">
          <el-input v-model="ruleForm.jiancaimingcheng" 
              placeholder="建材名称" clearable  :readonly="ro.jiancaimingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="建材名称" prop="jiancaimingcheng">
              <el-input v-model="ruleForm.jiancaimingcheng" 
                placeholder="建材名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
                                    <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="建材类型" prop="jiancaileixing">
          <el-select v-model="ruleForm.jiancaileixing" placeholder="请选择建材类型">
            <el-option
                v-for="(item,index) in jiancaileixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="建材类型" prop="jiancaileixing">
	      <el-input v-model="ruleForm.jiancaileixing"
                placeholder="建材类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
                                    <el-col :span="24">  
...
export default {
  data() {
    ...
    return {
      ...
      type: '',
      ro:{
	jiancaimingcheng : false,
	jiancaileixing : false,
	tupian : false,
	danwei : false,
	zhongliang : false,
	faburiqi : false,
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          	            if(o=='jiancaimingcheng'){
            this.ruleForm.jiancaimingcheng = obj[o];
	    this.ro.jiancaimingcheng = true;
            continue;
          }
	            	            if(o=='jiancaileixing'){
            this.ruleForm.jiancaileixing = obj[o];
	    this.ro.jiancaileixing = true;
            continue;
          }
	            	            if(o=='tupian'){
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `jiancaishangpin/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jiancaishangpinCrossAddOrUpdateFlag = false;
                  this.parent.search();
