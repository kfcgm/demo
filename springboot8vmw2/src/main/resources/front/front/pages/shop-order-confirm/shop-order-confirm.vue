		...
		<form>
			...
			<view v-for="(item,index) in orderGoods " v-bind:key="index" class="cu-form-group">
				...
				<view class="title">
					...
					<view v-else>
						{{item.address}}号
						<text style="margin-left: 30upx;color: red;">￥{{item.price}}</text>
					</view>
				</view>
			</view>
						...
						if (res.confirm) {
							...
							for (let i = 0; i < _this.orderGoods.length; i++) {
								// 如果有限次模块 ，则减少商品的库存信息，不够则提示库存不足
                                                                                                                                                                                                                                                                                                // 当前商品是否相同表
                                if(_this.orderGoods[i].tablename == `jiancaishangpin`){
                                     // 获取数量 
                                     let res = await _this.$api.info('jiancaishangpin', _this.orderGoods[i].goodid);
                                     // 单次限次
                                     ...
                                     if(res.data.alllimittimes<_this.orderGoods[i].buynumber){
                                        ...
                                        break;
                                         ...
                                         // 库存充足则减少库存数量
                                         await _this.$api.update(`jiancaishangpin`,  res.data);
                                     }
                                }
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                								let order =  {
